...
The same lifetime bug exists when a stack address is retained in the native heap, a Go-managed heap object, a mutable global, or another object that can outlive the frame. Passing a stack pointer to a synchronous callee is not itself an escape if the callee does not retain it.
...
`-O0` preserves source shapes for this separate audit build, while `-gline-tables-only` gives the audit useful file and line locations. Generate the release assembly and manifest with your normal optimized command after the audit passes.
