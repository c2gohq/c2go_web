...
sourceLinks:
  ...
  - label: c2go-lto archive compatibility test
    url: https://github.com/c2gohq/c2go_clang/blob/v0.20260729.0-rc.5/llvm/test/tools/c2go-lto/wf2-archive-roundtrip.test
...
```text
.c ──c2go-clang -c -emit-llvm──▶ .o (LLVM bitcode)
...
$(C2GO_BUILD)/%.o: .c2go-src/%.c
	...
	$(C2GO_CLANG) --target=$(C2GO_TARGET) \
		...
		$(C2GO_CPPFLAGS) $(C2GO_CFLAGS) -emit-llvm \
		-MMD -MP -MF $(@:.o=.d) -MT $@ \
		-c $< -o $@
...
The `-emit-llvm` flag is required for this route; `-flto` is an equivalent way to make Clang write bitcode-wrapped `.o` files. A plain `-fc2go -c` without either flag currently writes a native ELF/Mach-O/COFF object, which `c2go-lto` cannot consume.
...
make libengine.a \
  ...
  AR=c2go-lto \
  RANLIB=: \
  CFLAGS="--target=$C2GO_TARGET -fc2go -fc2go-package=$C2GO_PACKAGE -O2 -emit-llvm"
...
CMake can keep its normal `source → .o → static library` graph too. Setting only `CMAKE_C_COMPILER=c2go-clang` is not sufficient: the target must emit bitcode objects, the archive creation rule must call `c2go-lto`, the `ranlib` finish step must be disabled, and the resulting archive must be passed to `c2go-bind`.
