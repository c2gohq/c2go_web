...
The coordinated release is intentionally narrow:
...
- no automatic promotion when a C stack pointer escapes into heap or global storage; and
- pre-1.0 APIs and ABI details that may still change.

The release page lists the host SDK archives that are actually built and tested. Do not infer a supported release package merely because a backend can generate code for a target.
...
It is not yet a drop-in replacement for every cgo project. C++ code, 32-bit targets, dynamic stack allocation, packed managed records, stack-to-heap pointer escape, and several platform-specific callback shapes remain outside the current contract. Read and enforce the [stack escape safety gate](/en/docs/stack-escape-audit/) before porting stateful code.

## Next step

Install the matching SDK and Go runtime module, then compile one exported C function in [Hello World](/en/docs/hello-world/).
