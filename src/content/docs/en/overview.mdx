...
For a C translation unit, `c2go-clang` produces:

- Plan 9 assembly (`.s`) for the Go assembler;
- a JSON manifest describing exported symbols, types, target information, callbacks, and linker inputs; and
- an incidental object file when the driver is invoked with `-c`.
...
C2Go adds an explicit distinction between two memory worlds:

- **managed** values participate in the Go runtime's memory model and are the default;
...
It is not yet a drop-in replacement for every cgo project. C++ code, 32-bit targets, dynamic stack allocation, packed managed records, and several platform-specific callback shapes remain outside the current contract.
