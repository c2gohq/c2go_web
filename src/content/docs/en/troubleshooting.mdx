...
Delete stale generated inputs and rebuild the `.s` and JSON manifest together. Check that `-fc2go-package` equals `--pkg`, and never reuse an output filename across targets in a shared build directory.
...
Confirm the declaration is `unmanaged extern`, the symbol is actually referenced, and the binder receives the correct `-l` and `-L` options. Verify the library matches the target architecture. On Windows, also check the exact exported symbol spelling.
