...
C2Go 明确区分两个内存世界。当前实现的 translation unit 默认是 **unmanaged**；需要通过 record/pointer 属性或 `#pragma c2go managed(N) push` 区域显式选择 managed 行为：
...
它目前还不能无差别替代所有 cgo 项目。C++、32 位目标、动态栈分配、packed managed record、stack-to-heap pointer escape，以及部分平台相关回调形状仍不在当前契约内。迁移带状态代码前，请先阅读并落实[栈指针逃逸门禁](/zh-cn/docs/stack-escape-audit/)。
